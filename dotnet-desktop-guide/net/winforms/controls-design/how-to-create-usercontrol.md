...
In this article you'll learn how to add a user control to your project, and then add that user control to a form. You'll create a reusable user control that's both visually appealing and functional. The control groups a <xref:System.Windows.Forms.TextBox> control with a <xref:System.Windows.Forms.Button> control. When the user clicks the button, the text in the text box is cleared. For more information about user controls, see [User control overview (Windows Forms .NET)](usercontrol-overview.md).
...
The user control is made up of _constituent controls_, which you [drag-and-drop controls on the designer](../controls/how-to-add-to-a-form.md), just like how you design a form. Follow these steps to add and configure the control and its constituent controls:

01. With the designer open, the user control should be the focused object. If it's not, click on the design surface to select it. Set the following properties:
...
01. Finally, build the project by right-clicking the project in the **Solution Explorer** pane, and selecting **Build**. There shouldn't be any errors, and after the build is finished, the `ClearableTextBox` control appears in the toolbox for use.
...
01. Open the **Toolbox** pane, and double-click the **ClearableTextBox** control. This control should be listed under a section named after your project, and in the example being used to write this article, the project's name is **UserControlProject**.
