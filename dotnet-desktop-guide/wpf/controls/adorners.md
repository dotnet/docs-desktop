...
| Title | Description |
|-------|-------------|
| [Implement an Adorner](how-to-implement-an-adorner.md) | Shows how to create a custom adorner implementation. |
| [Bind an Adorner to an Element](how-to-bind-an-adorner-to-an-element.md) | Demonstrates how to attach an adorner to a specific UI element. |
| [Adorn the Children of a Panel](how-to-adorn-the-children-of-a-panel.md) | Shows how to apply adorners to all child elements of a panel. |
| [Remove an Adorner from an Element](how-to-remove-an-adorner-from-an-element.md) | Explains how to programmatically remove an adorner. |
| [Remove all Adorners from an Element](how-to-remove-all-adorners-from-an-element.md) | Shows how to clear all adorners from an element at once. |

## Styles and templates

The <xref:System.Windows.Documents.Adorner> class is an abstract base class that doesn't define template parts or visual states. Concrete adorner implementations inherit from this class and define their own rendering behavior through the <xref:System.Windows.UIElement.OnRender%2A> method.
...
Understanding adorners requires familiarity with several key concepts about how they work within the WPF framework.
...
It is important to note that adorners do not include any inherent rendering behavior; ensuring that an adorner renders is the responsibility of the adorner implementer. A common way of implementing rendering behavior is to override the <xref:System.Windows.UIElement.OnRender%2A> method and use one or more <xref:System.Windows.Media.DrawingContext> objects to render the adorner's visuals as needed.
...
The following examples demonstrate how to implement and use adorners in common scenarios.
...
1. Call the <xref:System.Windows.Documents.AdornerLayer.Add%2A> method to bind the adorner to the target <xref:System.Windows.UIElement>.
...
1. Enumerate through the children of the parent element and call the <xref:System.Windows.Documents.AdornerLayer.Add%2A> method to bind an adorner to each child element.
