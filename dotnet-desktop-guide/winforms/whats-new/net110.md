...
- <xref:System.Windows.Forms.Clipboard.GetDataObject> and <xref:System.Windows.Forms.DataObject.GetImage> return bitmap images placed on the clipboard. `DataObject.GetImage` checks the typed clipboard pipeline first, and falls back to the legacy path so the behavior matches <xref:System.Windows.Forms.Clipboard.GetImage>.
