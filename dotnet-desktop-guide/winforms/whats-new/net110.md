...
In .NET 11 Preview 4, <xref:System.Windows.Forms.Clipboard.GetDataObject> and <xref:System.Windows.Forms.DataObject.GetImage> once again return bitmap images placed on the clipboard. `DataObject.GetImage` now checks the typed clipboard pipeline first, and then falls back to the legacy path so the behavior matches <xref:System.Windows.Forms.Clipboard.GetImage>.
