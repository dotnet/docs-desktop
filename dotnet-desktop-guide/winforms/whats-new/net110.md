---
title: What's new in WinForms for .NET 11 RC 1
description: Learn about what's new in Windows Forms for .NET 11 RC 1. New versions of Windows Forms are released yearly with .NET.
ms.topic: whats-new
ms.date: 09/17/2026
...
<xref:System.Windows.Forms.KioskModeManager> gives you a straightforward way to put a form into a kiosk or shared-screen experience without custom window-management logic. It can preserve a form's state across full-screen transitions, prevent display and system sleep, hide the pointer after inactivity, and respond to keyboard shortcuts for toggling full screen. The component accepts a <xref:System.Windows.Forms.ContainerControl>, so it can be used on a <xref:System.Windows.Forms.Form> or <xref:System.Windows.Forms.UserControl> and resolve the containing form at run time.
...
<xref:System.Windows.Forms.KioskModeManager.FullScreen?displayProperty=nameWithType> can also be data-bound, and <xref:System.Windows.Forms.KioskModeManager.FullScreenChanged?displayProperty=nameWithType> reports transitions between full-screen and restored states. <xref:System.Windows.Forms.KioskModeManager.ToggleFullScreen?displayProperty=nameWithType> switches directly between the two states.
...
> [!IMPORTANT]
> Rendering behavior for Visual Styles and the associated new APIs isn't final yet and continues to change until .NET 11 reaches GA. The behavior of the new APIs might likewise change in detail or in specific scenarios.
>
...
- The preview-only `TreeView.NodeLeading` property, `TreeView.NodeLeadingChanged` event, and protected `TreeView.OnNodeLeadingChanged(EventArgs)` method have been removed. Apps compiled against .NET 11 previews should remove uses of these APIs before moving to RC 1, and use `TreeView.ItemHeight` when an explicit node height is required.

- Animated `PictureBox` frame callbacks now stop when the control has no window handle, before evaluating `InvokeRequired`. This avoids intermittent handle-creation failures during animation callback timing and narrows the callback lifecycle change to that timing window.
...
- **Dark mode**
  ...
  - `LinkLabel` contrast in Dark Mode is improved. `TabControl` nested in `SplitContainer` now matches dark mode. `HelpProvider` text is now visible in dark mode. `ToolTip` now switches to dark mode when `SystemColorMode.Dark` is set.

  - Forms now preserve their dark title bar when a right-to-left layout change recreates the handle, and `ErrorProvider` tooltips and `TreeView`/`ListView` hover tooltips render with dark colors.

- **Clipboard**: automatic clipboard format conversion is restored for reads, including correct Unicode text conversion.

- **Controls**: right-to-left multiline `TextBox` controls render the vertical scroll bar correctly, `TaskDialog` handles unexpected button IDs without throwing, and empty `PrintDocument` values now show a blank preview instead of a black page.

- **Drawing and high DPI**: `ControlPaint.Light` interpolation for system colors is corrected, and `PropertyGrid` icon scaling is fixed on a 100% secondary display.
