...
- <xref:System.Windows.Forms.Clipboard.GetDataObject> and <xref:System.Windows.Forms.DataObject.GetImage> return bitmap images placed on the clipboard. <xref:System.Windows.Forms.DataObject.GetImage> now retrieves images through the typed clipboard pipeline by calling `TryGetData<Image>(DataFormats.Bitmap, autoConvert: true, ...)` and returns `null` when it can't retrieve an image.
