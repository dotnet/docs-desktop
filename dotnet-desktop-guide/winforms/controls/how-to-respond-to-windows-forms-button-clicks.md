...
- In the button's `Click` <xref:System.EventHandler> write the code to run. `Button1_Click` must be bound to the control. For more information, see [How to: Create Event Handlers at Run Time for Windows Forms](/dotnet/desktop/winforms/controls/how-to-add-an-event-handler#handle-an-event-at-runtime).  
