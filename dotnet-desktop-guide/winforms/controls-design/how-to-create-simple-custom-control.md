...
This article shows you how to create a custom Windows Forms control. The simple control you develop in this article prints the control's <xref:System.Windows.Forms.Control.Text> to the left, center, or right of the control. You can change the alignment of the text. This control doesn't raise or handle events.
...
Create a new property on the control named `TextAlignment`. This property adjusts where the control paints the text. In the `FirstControl` class, perform the following steps:
...
01. Add the following attributes to the property to integrate it with the **Properties** window in Visual Studio.
    ...
    - `DefaultValue`&mdash;A default value for the property.
      ...
      The default value enables the designer to **reset** the property. It also helps determine when the property should be serialized to the code-behind, as default values aren't serialized.
...
The `TextAlignment` property calls `Invalidate` so that the control repaints itself. This action ensures that the control uses the correct alignment when rendering the `Text`. However, if the `Text` property changes, the control doesn't update because `Text` doesn't call `Invalidate`. The property does call the `OnTextChanged` method, though, which you can override to call `Invalidate` and force the control to repaint itself.

In the `FirstControl` class, perform the following steps:
...
The last part of the custom control is painting. In the `FirstControl` class, perform the following steps:
...
01. Draw the `Text` property by using `Graphics.DrawString`.
